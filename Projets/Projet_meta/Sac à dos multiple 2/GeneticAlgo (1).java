...
public class GeneticAlgo {
	 ...
	 public sol generation_sol2(Sac‡dos[] sacs, objet[] objets)  {
	 ...
	 */
	 ...
	 void entasser(sol [] arr, int n, int i) {
		...
		public  void trie(sol [] arr) {
		    ...
		    // Extrait les √©l√©ments un par un du tas
		    for (int i = n - 1; i > 0; i--) {
		        // √âchange la racine actuelle avec le dernier √©l√©ment non tri√©
		        sol temp = arr[0];
		        arr[0] = arr[i];
		        arr[i] = temp;
		        entasser(arr, i, 0);
	 ...
	 /* 1tous ok =>ok
	    ...
	    // pour le croisement on choisie aleatoirement 2 sacs pour faire croisÈ apres 
	...
	/* boolean verifier(ArrayList<Sac> s) {
	 ...
	 public sol[] croisement (sol[] pop,float r) {
		 ...
		 if(nbparent==pop.length) {
			...
			sol [] croisement =new sol[nbparent];//pour resultat retournÈ
...
//********************************************************************************************************
							...
							// ici on ne soucie pas de duplication mais par la validitÈ
					  ...
					  if(o.valeur<=v && o.poids>=po ) {
						 ...
						 parent2.get(position2).poidcour=comptervalc(parent2.get(position2).objets);
						  ...
						  sol1.sacs.addAll(parent1);
							...
							//ici si le poids s'est pas reglÈ donc on prends poids ki affecte le min possible le v 
							if(parent2.get(position2).poidsmax<parent2.get(position2).poidcour) {
	...
	//********************************************************************************************************							
			...
			if(pos.size()<parent1.get(position1).objets.size() && pos1.size()<parent2.get(position2).objets.size()) {
						...
						// obj parent1 qui ne sont pas pos1 seront stockÈ objets restant 
	 ...
	 public sol[] mutation(sol[] croisement,float rate) {
		 ...
		 croisement =verifierfitness(croisement,nbrmutation);
		 ...
		 while(nbr<nbrmutation) {
			 ...
			 //sacs cest l'individu qu'on va le passer au param a nv pour la muter
				...
				int p2;
				int p1;
				 p1=random.nextInt(nv.sacs.size());
				do {
				 p2=random.nextInt(nv.sacs.size());
				} while(p1==p2);
		...
		public sol [] remplacer(sol[] pop,sol[] croisement,sol[] mutation) {
			
			int taille=pop.length+croisement.length+mutation.length;
			sol[] temp=new sol[taille];
			System.arraycopy(pop, 0, temp, 0, pop.length);
			System.arraycopy(croisement, 0, temp, pop.length, croisement.length);
			System.arraycopy(mutation, 0, temp, pop.length + croisement.length, mutation.length);
			trie(temp);
			temp =verifierfitness(temp,temp.length);
			System.arraycopy(temp, 0, pop, 0, pop.length);
			return pop;
		}
	 
	 
	 public void genetique_algo(int maxgen,int nbr_pop,Sac‡dos[] sacs, objet[] objets) {
	  ...
	  double tempsEcouleEnSecondes = duration / 1_000_000_000.0; 
		...
		for(Sac s :pop[0].sacs) {
			System.out.println("sac : poids max : "+s.poidsmax+" valmin : "+s.valmin+" valcour : "+s.valcour+"poid cour : "+s.poidcour);
			for(objet o :s.objets) {
				System.out.print("num objet: "+o.num+" poids objet : "+o.poids+"val objet :"+o.valeur);
				
			}
		}
		for(int i=0;i<pop[0].sacs.size();i++) {
			System.out.print(pop[0].sacs.get(i));
		}
		System.out.println("");
		System.out.println(pop[0].valeur_total);
	 }
